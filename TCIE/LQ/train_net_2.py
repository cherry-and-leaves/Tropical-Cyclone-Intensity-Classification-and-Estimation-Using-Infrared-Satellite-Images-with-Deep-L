...
from define_net_WXJ import Net_110
from define_net_WXJ import Net_170
from define_net_WXJ import Net_170_WXJ, Net_170_conv5, Net_170_conv5_256,Net_170_conv4
from define_net_WXJ import Net_40_WXJ
from define_net_WXJ import Net_200
from define_net_WXJ import Net_230
from define_net_WXJ import Net_20
from define_net_WXJ import Net
from define_net_WXJ import Net_80
from define_net_WXJ import Net_50
from define_net_WXJ import Net_256
...
if __name__ == '__main__':
    ...
    trainset = ImageFolder(path_+'/4and5_256/', imgsize, transform)#4and5_256_ZJ_really
    # trainset = ImageFolder(path_ + '/4and5_256_ZJ_really/', imgsize, transform)  # 4and5_256_ZJ_really
    ...
    pathoutput = r"D:\1WXJ\Estimate\Model_2\A_2_noZJ_BS=128"#
    pathlosssave = os.path.join(r'D:\1WXJ\Estimate\plot_2\A_2_noZJ_BS=128')#
    ...
    net = Net_40_WXJ()
    # net.fc = nn.Sequential(nn.Linear(6400, 80),
    #                        nn.ReLU(),
    #                        # nn.Dropout(p=0.2),
    #                        nn.Linear(80, 16),
    #                        nn.ReLU(),
    ...
    init.xavier_uniform(net.conv2_2.weight.data, gain=1)
    init.constant(net.conv2_2.bias.data, 0.1)
    init.xavier_uniform(net.conv3.weight.data, gain=1)
    init.constant(net.conv3.bias.data, 0.1)
    init.xavier_uniform(net.conv3_2.weight.data, gain=1)
    init.constant(net.conv3_2.bias.data, 0.1)
...
#定義損失函數·和分類器
    ...
    for epoch in range(500):
        ...
        for i,data in enumerate(trainloader, 0):
            ...
            optimizer.zero_grad()
            outputs = net(inputs)
            loss1 = criterion1(outputs,labels.float())
        ...
        totalloss.append(running_loss / (i + 1))
        ...
        for i, data in enumerate(testloader, 0):
            ...
            inputs, labels = Variable(inputs.cuda()), Variable(labels.cuda())
            outputs = net(inputs)
            test_loss = criterion1(outputs, labels.float())+ criterion2(outputs, labels.float())
