...
file = os.path.join(path_, r'error_percent_1830.xlsx')
outfile = os.path.join(path_, r'error_percent_1830.csv')
def xlsx_to_csv_pd():
...
if __name__ == '__main__':
    ...
    hw1 = pd.read_csv(os.path.join(path_, r'error_percent_1830.csv'))
    # 转置
    hw1.values
    data = hw1.as_matrix()
    data = list(map(list, zip(*data)))
    data = pd.DataFrame(data)
    data.to_csv(os.path.join(path_, r'error_percent_ZZ.csv'),header='AE', index=0)
    filename = os.path.join(path_, r'error_percent_ZZ.csv')  # CSV文件路径
    lines = []
    ...
    plt.savefig(os.path.join(path_, r'PIC\error_1830.tif'), dpi=900)
    plt.savefig(os.path.join(path_, r'PIC\error_1830.eps'), dpi=900)
    plt.show()
